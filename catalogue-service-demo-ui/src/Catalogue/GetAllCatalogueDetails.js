...
const GetAllCatalogueDetails = () => {
    ...
    const [viewGetCatalogue, setViewGetCatalogue] = useState('');
    const [isCopied, setIsCopied] = useState(false);
    const [isCopied1, setIsCopied1] = useState(false);
    function getAllCatalogue() {
        var getCatalogue = {
            FunctionName: 'aws-node-http-api-project-dev-get_all_catalogues_details',
            Payload: getAllInput
        };
        lambda.invoke(getCatalogue, function (err, data) {
            if (err) console.log(err, err.stack);
            else {
                setViewGetCatalogue(JSON.parse(data.Payload));
            };
        });
    }
    const onCopyText = () => {
    ...
    function PrettyPrint() {
        const a = JSON.parse(getAllInput);
        setgetAllInput(JSON.stringify(a, null, 2))
    }
    return (
        <>
            <center>
            ...
            <div className='d-flex justify-content-around'>
                <div className='d-flex flex-column myBox' style={{ width: "600px" }}>
                    <h6>Get All Catalogue_Details</h6>
                    <textarea className='form-control' value={getAllInput} onChange={e => setgetAllInput(e.target.value)} style={{ height: "300px", fontSize: "10px" }} placeholder="Input Here" />
                    <div className='d-flex'>
                        <button className='btn btn-info m-2 mt-3' style={{ width: "50%" }} onClick={getAllCatalogue}>Get-All Input</button>
                ...
                <div className='d-flex flex-column myBox' style={{ width: "800px" }}>
                    ...
                    <div style={{ marginLeft: "95%", marginTop: "-40px" }}>
                        <CopyToClipboard text={JSON.stringify(viewGetCatalogue)} onCopy={onCopyText1}>
                    ...
                    <div class="scrollbar" id="style-default">
                        <div class="force-overflow">
                            <div class="modal-body">
                                <pre>{JSON.stringify(viewGetCatalogue, null, 2)}</pre>
                            </div>
                        </div>
                    </div>
                </div>
...
export default GetAllCatalogueDetails;