...
const AppendKeyCatalogueDetails = () => {
    ...
    function appendJsonCatalogue() {
        var appendServiceCatalogue = {
            FunctionName: 'aws-node-http-api-project-dev-append_key_catalogues_details',
            Payload: append
        };
        lambda.invoke(appendServiceCatalogue, function (err, data) {
            if (err) console.log(err, err.stack);
            else {
                console.log(data)
                // console.log("from append input", JSON.parse(data.Payload).rows);
                setViewAppend(JSON.parse(data.Payload));
            };
        });
    }
    const onCopyText = () => {
        ...
        <>
            ...
            <div className='d-flex justify-content-around'>
                <div className='d-flex flex-column myBox' style={{ width: "600px" }}>
                    <h6>Append Key Catalogue_Details</h6>
                    ...
                    <div className='d-flex'>
                        <button className='btn btn-info m-2 mt-3' style={{ width: "50%" }} onClick={appendJsonCatalogue}>Append Key Input</button>
...
export default AppendKeyCatalogueDetails;