exports.deactivate_purchase_order = async (event, context, callback) => {
    ...
    let objReturn = {
        code: 200,
        message: "Purchase order successfully deactivate",
        type: "object",
        object: []
    };
    ...
    try {
        ...
        if (event.id) {
            const res = await client.query(`UPDATE purchase_order SET details = jsonb_set(details, '{status}', '"DEACTIVATED"') WHERE id=$1`, [event.id]);
