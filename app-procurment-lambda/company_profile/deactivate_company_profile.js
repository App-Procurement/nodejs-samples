exports.deactivate_company_profile = async (event, context, callback) => {
    ...
    let objReturn = {
        code: 200,
        message: "company_profile successfully deactivate",
        type: "object",
        object: []
    };
    ...
    try {
        ...
        if (event.id) {
            const res = await client.query(`UPDATE company_profile SET details = jsonb_set(details, '{status}', '"DEACTIVATED"') WHERE id=$1`, [event.id]);
