exports.upadte_approvers = async (event, context, callback) => {
    event = event.body
    ...
    try {
        ...
        if (event.id) {
            ...
            const res = await client.query(`UPDATE approvers SET details= $1::jsonb WHERE id = $2`, [event.details, event.id]);
