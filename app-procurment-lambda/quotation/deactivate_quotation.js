exports.deactivate_quotation = async (event, context, callback) => {
    ...
    let objReturn = {
        code: 200,
        message: "quotation successfully deactivate",
        type: "object",
        object: []
    };
    ...
    try {
        ...
        if (event.id) {
            const res = await client.query(`UPDATE quotation SET details = jsonb_set(details, '{status}', '"DEACTIVATED"') WHERE id=$1`, [event.id]);
